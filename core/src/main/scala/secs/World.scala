...
trait World:
  ...
  def insertComponent[C <: Component](entity: Entity, component: C)(using CM: ComponentMeta[C]): Unit
  def updateComponent[C <: Component](entity: Entity, update: C => C)(using CM: ComponentMeta[C]): Unit
  def removeComponent[C <: Component](entity: Entity)(using CM: ComponentMeta[C]): Unit
...
given World with
  ...
  def despawnEntity(entity: Entity) =
    entities.get(entity).foreach(_.keys.foreach { cm =>
    ...
    entities -= entity
  def insertComponent[C <: Component](entity: Entity, component: C)(using CM: ComponentMeta[C]) =
    entities.get(entity).foreach { cs =>
      entities += entity -> (cs + (CM -> component))
      components = components.updatedWith(CM)(_.fold(Some(Set(entity)))(es => Some(es + entity)))
    }
  def updateComponent[C <: Component](entity: Entity, update: C => C)(using CM: ComponentMeta[C]) =
    entities.get(entity).foreach { cs =>
      entities += entity -> cs.updatedWith(CM)(_.map(update.asInstanceOf[Component => Component]))
    }
  def removeComponent[C <: Component](entity: Entity)(using CM: ComponentMeta[C]) =
    entities.get(entity).foreach { cs =>
